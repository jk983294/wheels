...
TEST_CASE("SkipList InsertAndLookup", "[SkipList]") {
  const int N = 2000;
  const int R = 5000;
  Random rnd(1000);
  std::set<Key> keys;
...
TEST_CASE("SkipList ConcurrentWithoutThreads", "[SkipList]") {
  ConcurrentTest test;
...
TEST_CASE("SkipList Concurrent", "[SkipList]") {
  RunConcurrent(1);
  RunConcurrent(2);
  RunConcurrent(3);
  RunConcurrent(4);
