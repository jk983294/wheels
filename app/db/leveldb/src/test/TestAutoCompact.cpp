...
void AutoCompactTest::DoReads(int n) {
  ...
  REQUIRE(final_other_size <= initial_other_size + 1048576);
  REQUIRE(final_other_size >= initial_other_size / 5 - 1048576);
}

TEST_CASE("AutoCompactTest ReadAll", "[AutoCompactTest]") {
  AutoCompactTest test;
  test.DoReads(kCount);
}

TEST_CASE("AutoCompactTest ReadHalf", "[AutoCompactTest]") {
  AutoCompactTest test;
  test.DoReads(kCount / 2);
}
