...
std::vector<std::string> inline_tags = {"b",    "big", "i",      "small", "tt",     "abbr",   "acronym", "cite",
                                        "code", "dfn", "em",     "kbd",   "strong", "samp",   "time",    "var",
                                        "a",    "bdo", "br",     "img",   "map",    "object", "q",       "span",
                                        "sub",  "sup", "button", "input", "label",  "select", "textarea"};

std::vector<std::string> void_tags = {"area",  "base", "br",   "col",   "embed",  "hr",    "img",
                                      "input", "link", "meta", "param", "source", "track", "wbr"};

std::vector<std::string> rawtext_tags = {"title",     "textarea", "style", "script",  "noscript",
                                         "plaintext", "iframe",   "xmp",   "noembed", "noframes"};
...
selector::selector_matcher::selector_matcher(selector_matcher&& m)
    : all_match(m.all_match), conditions(std::move(m.conditions)) {
    m.all_match = false;
    m.conditions.clear();
}
...
bool selector::condition::operator()(const node& d) const {
    ...
    if (!attr.empty()) {
        ...
        if (attr_operator == "=") {
            ...
            return attr_value.size() <= it->second.size() &&
                   it->second.find(attr_value) == it->second.size() - attr_value.size();
        } else if (attr_operator == "!=") {
            return it->second != attr_value;
        } else if (attr_operator == "*=") {
...
void node::walk(std::function<bool(node&)> handler) { walk(*this, handler); }
...
void node::to_html(std::ostream& out, bool child, bool text, int level, int& deep, char ind, bool& last_is_block,
                   bool& sibling_is_block) const {
    ...
    if (type_node == node_t::none) {
        ...
        if (text && std::any_of(content.begin(), content.end(), [](char c) { return !IS_SPACE(c); })) {
            auto str = content;
...
void node::set_attr(const std::string& key, const std::string& val) { attributes[key] = val; }
    ...
    if (new_node_ptr->type_node == node_t::tag) {
        if (new_node_ptr->type_tag == tag_t::open) {
            ...
            if (!new_node_ptr->self_closing) {
                ...
                } else if (std::find(rawtext_tags.begin(), rawtext_tags.end(), new_node_ptr->tag_name) !=
                           rawtext_tags.end()) {
