...
enum class node_t { none, text, tag, comment, doctype };
...
enum class err_t { tag_not_closed };
...
public:
    ...
    std::vector<node_ptr>::iterator begin() { return children.begin(); }
    std::vector<node_ptr>::iterator end() { return children.end(); }
...
public:
    ...
    operator bool() const { return !matchers.empty(); }
...
private:
    ...
    std::vector<selector_matcher>::const_iterator begin() const { return matchers.begin(); }
    std::vector<selector_matcher>::const_iterator end() const { return matchers.end(); }
